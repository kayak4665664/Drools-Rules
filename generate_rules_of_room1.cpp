...
struct members {
    int sum, when_sum, then_sum;
};

bool cmpwwhens(pair<int, members> a, pair<int, members> b) {
    return a.second.when_sum > b.second.when_sum;
}
...
auto main() -> int {
    ...
    for (int i = rules.size(); i < amt; i = rules.size()) {
        ...
        while (true) {
            ...
            if (!cntpeo) {
                ...
                vector<pair<int, members>> wwwhen;
                for (auto iter = wwhen.begin(); iter != wwhen.end(); ++iter)
                    wwwhen.push_back(*iter);
                stable_sort(wwwhen.begin(), wwwhen.end(), cmpwwhens);
                for (auto it = wwwhen.begin(); it != wwwhen.end(); ++it) {
                    int cls = it->first, sum = it->second.sum, cdts; /*
                     if (cth++ == theone)
                         it->second.when_sum = rand() % sum + 1;
                     else
    ...
    ofstream ofs;
    ofs.open("smartRoom" + to_string(amt) + ".drl", ios::out);
